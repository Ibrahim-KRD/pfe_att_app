...
interface EnrollementDao {
    ...
    @Transaction
    @Query(
        "SELECT enrollments.*, Seance.*, Student.* FROM enrollments " +
    ...
    fun getEnrollmentWithSeanceStudent(
        studentId: Int,
        seanceId: Int
    ): EnrollmentWithSeanceStudentModule?

    @Query("Select * from enrollments where id = :enrollment_id")
    fun getEnrollement(enrollment_id: Int): Enrollment?
